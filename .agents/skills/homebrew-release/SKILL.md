...
| --- | --- |
| macOS Apple Silicon | `google-analytics-gateway-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `google-analytics-gateway-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/google-analytics-gateway.rb
```
...
brew install user/tap/google-analytics-gateway
google-analytics-gateway --version
brew test user/tap/google-analytics-gateway
```
...
verify `api/formula/google-analytics-gateway.json` from
