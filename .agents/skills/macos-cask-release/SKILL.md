...
```bash
brew tap user/tap
brew install --cask google-analytics-gateway
```
...
swift run google-analytics-gateway --version | tail -n 1 | grep -Fx "$version"
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-analytics-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-analytics-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-analytics-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-analytics-gateway-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/google-analytics-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/google-analytics-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release google-analytics-gateway <version>"
...
verify `api/cask/google-analytics-gateway.json` from GitHub
