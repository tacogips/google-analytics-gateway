...
public struct CapabilityRegistry: Sendable {
  ...
  /// Every Google service the linked capabilities address.
  public var services: [GoogleAPIService] {
    Array(Set(definitions.map(\.service))).sorted()
  }
  ...
  public func coherenceProblems() -> [String] {
    ...
    for definition in definitions {
      ...
      if !GoogleHostPolicy.production.allows(host: definition.service.host) {
        problems.append(
          "Capability \(definition.id) addresses \(definition.service.host), "
            + "which the production host policy does not approve."
        )
      }
    }
    return problems
