...
public struct GatewayError: Error, Sendable, Equatable {
  ...
  /// True when a non-idempotent operation failed in a way that cannot prove the
  /// Google API did not apply it. Callers must not assume failure or success.
  ...
  public var extensions: JSONValue {
    var fields: [String: JSONValue] = ["code": .string(code.rawValue)]
