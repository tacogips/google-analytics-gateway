...
struct GraphQLLexer {
  ...
  /// property display name, is escaped in GraphQL as a surrogate pair. Reading
  /// each half on its own yields no scalar, so a document that is valid under
  /// the contract would be rejected for a syntax it is required to accept.
