...
public struct GraphQLResponse: Sendable, Equatable {
  ...
  public init(data: JSONValue?, errors: [GatewayError], requestID: String) {
    self.data = data
    self.errors = errors
  ...
  public var stableValue: JSONValue {
    var envelope: [String: JSONValue] = ["data": data ?? .null]
    if !errors.isEmpty {
...
public struct GraphQLRuntime: Sendable {
  ...
  public func execute(document: String, variables: [String: JSONValue] = [:]) async -> GraphQLResponse {
  ...
  func prepare(_ document: GraphQLDocument, variables: [String: JSONValue]) throws -> [PreparedField] {
