...
extension JSONValue {
  /// Decodes untrusted upstream or CLI-supplied JSON into a `JSONValue`.
  ...
  /// `UPSTREAM_RESPONSE_INVALID` that `decodeJSON` reports for a Google API body.
  public static func decodeJSONObject(_ data: Data, context: String) throws -> [String: JSONValue] {
    let value: JSONValue
    do {
      ...
      throw GatewayError.validation(
        ...
        recovery: "Supply a JSON object such as {\"name\": \"properties/123456789\"}."
      )
    }
  ...
  private static func convert(number: NSNumber) -> JSONValue {
    #if canImport(Darwin)
  ...
  private static func write(_ value: JSONValue, into output: inout String, pretty: Bool, indent: Int) {
