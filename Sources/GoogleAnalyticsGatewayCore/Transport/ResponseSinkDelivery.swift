...
/// Turns a raw upstream answer into an `UpstreamResponse` that honours the
...
public enum ResponseSinkDelivery {
  /// Google error envelopes are small. Reading more than this from a failed
  /// download would let an unexpected upstream body be buffered whole, so the
  /// surplus is discarded and the mapper works from the prefix.
  public static let maximumBufferedErrorBytes = 64 * 1024

  /// Permissions applied to a written body. Downloaded content is account data,
  /// so it is readable only by the account that asked for it.
  ...
  public static func deliver(
    ...
    guard case .file(let path) = sink, ResponseSink.isSuccess(status: statusCode) else {
      return UpstreamResponse(statusCode: statusCode, headers: headers, body: body)
    }
  ...
  public static func deliver(
    ...
    guard ResponseSink.isSuccess(status: statusCode) else {
  ...
  ///
  /// A capability only selects a file sink for a route documented to return
  /// content, so a no-content answer is not a zero-byte body: it is a response
  /// that does not match the documented contract. Writing an empty file would
  /// report a success the caller could not distinguish from a real transfer, so
  /// no file is created and no `downloadedFile` is attached. The projection
  /// turns the missing description into `UPSTREAM_RESPONSE_INVALID`.
  ///
  /// The rule is deliberately limited to the statuses that *define* an absent
  /// body rather than to any empty success body, because content that is
  /// genuinely empty must still be delivered as a `200`.
  ...
  private static func noContentResponse(
    ...
    UpstreamResponse(statusCode: statusCode, headers: headers, body: Data())
  }
