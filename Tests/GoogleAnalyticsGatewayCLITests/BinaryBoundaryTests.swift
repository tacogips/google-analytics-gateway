...
///
/// `GoogleAnalyticsGatewayCLITests` depends on the three executable targets, so
/// `swift test` builds them before these tests run.
enum BuiltProducts {
  static let reader = "google-analytics-gateway-reader"
  static let writer = "google-analytics-gateway-writer"
  static let admin = "google-analytics-gateway-admin"
...
struct ExecutableLinkBoundaryTests {
  ...
  func readerExcludesHigherTiers() throws {
    ...
    #expect(symbols.contains("GoogleAnalyticsGatewayRead"), "Sanity check: the read module must be linked")
    #expect(!symbols.contains("GoogleAnalyticsGatewayWrite"), "Reader must not link the write module")
    #expect(!symbols.contains("GoogleAnalyticsGatewayAdmin"), "Reader must not link the admin module")
  }
  ...
  func writerExcludesAdmin() throws {
    ...
    #expect(symbols.contains("GoogleAnalyticsGatewayWrite"), "Sanity check: the write module must be linked")
    #expect(!symbols.contains("GoogleAnalyticsGatewayAdmin"), "Writer must not link the admin module")
  }
  ...
  func adminLinksAdmin() throws {
    ...
    #expect(symbols.contains("GoogleAnalyticsGatewayAdmin"))
  }
  ...
  func noTestSupportInProduction() throws {
    for name in BuiltProducts.all {
      ...
      #expect(!symbols.contains("GoogleAnalyticsGatewayTestSupport"), "\(name) must not link test support")
...
struct CrossTierSchemaTests {
  ...
  func cumulativeQueryFields() throws {
    ...
    for definition in registry.queryDefinitions {
  ...
  func schemaMatchesMetadata() throws {
    let expected: [TierExpectation] = [
      TierExpectation(binary: BuiltProducts.reader, tier: .reader, definitions: ReadCapabilities.all),
      TierExpectation(
        binary: BuiltProducts.writer, tier: .writer,
        definitions: ReadCapabilities.all + WriteCapabilities.all
      ),
      TierExpectation(
        binary: BuiltProducts.admin, tier: .admin,
        definitions: ReadCapabilities.all + WriteCapabilities.all + AdminCapabilities.all
      )
    ]
    for expectation in expected {
      let name = expectation.binary
      let registry = try CapabilityRegistry(
        tier: expectation.tier, definitions: expectation.definitions
      )
  ...
  func schemaDefinesEveryTypeItNames() throws {
    let builtInScalars: Set<String> = ["ID", "String", "Int", "Float", "Boolean", "JSON"]
    for name in BuiltProducts.all {
      let printed = try BuiltProducts.run(name, ["graphql", "schema"]).standardOutput
      let defined = Set(SchemaDocument.definedTypeNames(in: printed))
        .union(printed.contains("scalar JSON") ? ["JSON"] : [])
  ...
  @Test("Help is tier-scoped and never empty")
  ...
  func versionIsSingleLine() throws {
    for name in BuiltProducts.all {
      ...
      #expect(lines.first.map(String.init) == googleAnalyticsGatewayVersion, "\(name)")
    }
  }
}
...
enum SchemaDocument {
  ...
  static func referencedTypeNames(in document: String) -> [String] {
    ...
    for line in document.components(separatedBy: "\n") {
      ...
      // Docstrings hold prose; a colon there is punctuation, not type syntax.
      // A one-line docstring both opens and closes on the same line.
      let delimiters = trimmed.components(separatedBy: "\"\"\"").count - 1
      if delimiters % 2 == 1 {
        insideDocstring.toggle()
        continue
      }
      if insideDocstring || delimiters > 0 {
        continue
      }
...
struct ExecutableRuntimeBoundaryTests {
  ...
  func readerRefusesWrite() throws {
    let result = try BuiltProducts.run(BuiltProducts.reader, [
      "graphql", "query",
      "mutation { gtmCreateTag(parent: \"accounts/1/containers/2/workspaces/3\", tag: {name: \"x\", type: \"gaawc\"}) { path } }"
    ])
  ...
  func readerRefusesDelete() throws {
    let result = try BuiltProducts.run(BuiltProducts.reader, [
      "graphql", "query",
      "mutation { gaDeleteProperty(name: \"properties/1\", confirmName: \"properties/1\") { deletedName } }"
    ])
    #expect(result.exitCode == 2)
    #expect(result.standardOutput.contains("CAPABILITY_DENIED"))
  ...
  @Test("The writer names the admin tier for every admin mutation")
  func writerRefusesAdminMutations() throws {
    for field in CapabilityCatalog.adminMutationFields {
      let result = try BuiltProducts.run(BuiltProducts.writer, [
        "graphql", "query",
        "mutation { \(field)(name: \"accounts/1\") { deletedName } }"
      ])
  ...
  func validationBeforeCredentials() throws {
    let result = try BuiltProducts.run(BuiltProducts.reader, [
      "graphql", "query", "{ gaProperty(name: \"properties/1\") { notAField } }"
    ])
    #expect(result.exitCode == 2)
  ...
  func missingCredentialExitsThree() throws {
    let result = try BuiltProducts.run(BuiltProducts.reader, [
      "graphql", "query", "{ gaAccounts { nodes { name } } }"
    ])
    #expect(result.exitCode == 3)
    #expect(result.standardOutput.contains("AUTHENTICATION_FAILED"))
    #expect(result.standardOutput.contains(ProfileSelector.fallbackAccessTokenVariable))
  }

  @Test("auth status reports safe metadata only and never a token value")
  func authStatusIsSafe() throws {
    for name in BuiltProducts.all {
      ...
      #expect(lowered.contains("\"state\":\"missing\""), "\(name)")
  ...
  func streamSeparation() throws {
    let business = try BuiltProducts.run(BuiltProducts.reader, [
      "graphql", "query", "{ gaAccount(name: \"accounts/1\") { notAField } }"
    ])
  ...
  func prettyChangesWhitespaceOnly() throws {
    ...
    let pretty = try BuiltProducts.run(BuiltProducts.reader, [
      "--pretty", "graphql", "query", "{ gaAccount(name: \"accounts/1\") { notAField } }"
    ]).standardOutput
