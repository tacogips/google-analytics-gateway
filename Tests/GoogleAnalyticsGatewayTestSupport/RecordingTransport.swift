...
/// An in-memory `GoogleTransport` that records every prepared request and
/// answers with queued canned responses.
///
...
public actor RecordingTransport: GoogleTransport {
  ...
  public func firstRequest() throws -> RecordedRequest {
    guard let first = recorded.first else { throw TestSupportError.noRequestRecorded }
    return first
  }
    ...
    recorded.append(
      RecordedRequest(
        ...
        queryItems: (URLComponents(url: request.url, resolvingAgainstBaseURL: false)?.queryItems ?? [])
          .map { UpstreamQueryItem(name: $0.name, value: $0.value ?? "") },
  ...
  /// Describes a body for assertions. Only the rendered JSON document is kept;
  /// no credential can reach a request body, so nothing here can hold one.
  static func describe(_ body: UpstreamRequestBody) -> String {
    switch body {
    case .none:
      return "none"
    case .json(let value):
