...
Each selected product renders <output-dir>/<product>.rb. A legacy output path
ending in .rb is accepted and its parent directory is used instead.
...
Examples:
  ...
  scripts/render-homebrew-formula.sh 0.1.0 ../homebrew-tap/Formula
  scripts/render-homebrew-formula.sh 0.1.0 google-analytics-gateway-reader ../homebrew-tap/Formula
...
main() {
  ...
  release_base_url="${RELEASE_BASE_URL:-https://github.com/$homebrew_github_repository/releases/download/v$version}"
  ...
  for arg in "$@"; do
    if is_homebrew_product "$arg"; then
      ...
      if [[ "$arg" == google-analytics-gateway-* && "$arg" != */* ]]; then
      ...
      # One formula per product means the output is a directory. A single-file
      # path from the pre-split contract still resolves to its parent.
      if [[ "$arg" == *.rb ]]; then
